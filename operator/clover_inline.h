...
static inline void six_add_diag_const(_Complex double a[6][6], const double val) {
...
static inline void six_print(_Complex double a[6][6]) {
  for( unsigned int i = 0; i < 6; ++i ) {
    for( unsigned int j = 0; j <6; ++j) {
      printf("%.2e + i%.2e   ", (double) a[i][j], (double) *(&a[i][j]+sizeof(double)) );
    }
    printf("\n");
  }
  return;
